    ...
    for side_index in 0..2usize {
        ...
        for MeterRun { start, end, epoch } in runs_of(own, own_epoch, MeterState::Active) {
            ...
            fn event_from_run(
                ...
                let window_index = idx_at_or_before(features, window_end);
                let confidence = if continuous_epoch(own_epoch, start, window_index) == Some(epoch)
                    && continuous_epoch(opponent_epoch, start, window_index) == Some(epoch)
                {
...
mod unit_tests {
    ...
    #[test]
    fn frame_lookup_returns_the_preceding_nonzero_index() {
        let features = [feat(10, 1.0, 1.0), feat(20, 1.0, 1.0), feat(30, 1.0, 1.0)];
        assert_eq!(idx_at_or_before(&features, 25), 1);
    }
}
