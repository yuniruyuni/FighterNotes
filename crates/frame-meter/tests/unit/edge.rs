...
fn empty_row() -> (Vec<f32>, Vec<f32>, Vec<CellState>, Vec<BrightClass>) {
    (
        vec![0.0; CELL_COUNT],
        vec![0.0; CELL_COUNT],
...
fn edge_all_empty_returns_minus_one() {
    let (v, wf, states, bright) = empty_row();
    assert_eq!(fresh_color_edge(&v, &wf, &states, &bright), -1);
}

#[test]
fn edge_single_fresh_mid_with_gap() {
    let (mut v, wf, mut states, mut bright) = empty_row();
    // cell 5: Counter Fresh, v=200; cell 6: v=0（gap）
    v[5] = 200.0;
    states[5] = CellState::Counter;
    bright[5] = BrightClass::Fresh;
    assert_eq!(fresh_color_edge(&v, &wf, &states, &bright), 5);
}
...
fn edge_rightmost_wins() {
    ...
    // v[11], v[12] は 0 → gap
    assert_eq!(fresh_color_edge(&v, &wf, &states, &bright), 10);
}
...
fn edge_at_last_cell() {
    ...
    assert_eq!(fresh_color_edge(&v, &wf, &states, &bright), 79);
}
...
fn edge_stripe_inv_full_fresh_with_gap() {
    ...
    assert_eq!(fresh_color_edge(&v, &wf, &states, &bright), 20);
}
