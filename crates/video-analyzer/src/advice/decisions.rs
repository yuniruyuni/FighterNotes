...
pub(crate) enum DecisionSituation {
    /// ガード後に不利フレームを背負った。
    Disadvantage,
    /// ガードさせて有利フレームを取った。
    Advantage,
...
pub(crate) struct DecisionEvent {
    pub(crate) situation: DecisionSituation,
    pub(crate) frame: u32,
    /// 状況の大きさ（不利／有利フレーム数）。起き攻めは 0。
    pub(crate) frames: u32,
    pub(crate) option: DecisionOption,
    pub(crate) pressed: String,
    pub(crate) result: DecisionResult,
    pub(crate) drop: f32,
    pub(crate) round_no: u32,
}

/// 確度の高い判断機会だけを同じ形へ並べる。
pub(crate) fn collect_decisions(events: &MatchEvents, own: u8) -> Vec<DecisionEvent> {
    let mut out = Vec::new();
...
pub(crate) fn opportunities(decisions: &[DecisionEvent], situation: DecisionSituation) -> usize {
    decisions
        .iter()
...
pub(crate) fn losses<'a>(selected: &'a [&DecisionEvent]) -> Vec<&'a DecisionEvent> {
    selected
        .iter()
        .copied()
