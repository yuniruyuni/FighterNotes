...
pub(crate) fn detect_own_jumps(events: &MatchEvents, own: u8) -> Option<AdviceCard> {
    ...
    Some(AdviceCard {
        ...
        practice: if repeated {
            "落とされたジャンプの直前状況を確認し、接近手段をジャンプ以外に2つ（歩きガード・ドライブラッシュ等）用意します。相手が対空を見せた後は、同じ接近を連続で選ばない練習が有効です。"
        } else {
            "クリップでジャンプを選んだ理由を確認します。相手の対空を試した1回なら問題ありませんが、普段も同じ距離・同じタイミングで飛んでいる場合は接近手段を散らしましょう。"
        }.to_string(),
