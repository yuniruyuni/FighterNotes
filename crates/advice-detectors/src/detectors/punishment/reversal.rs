...
pub fn detect_reversal_punished(events: &MatchEvents, own: u8) -> Option<AdviceCard> {
    ...
    let mode = match (super_only, repeated) {
        (true, true) => ReversalCardMode::SuperRepeated,
        (true, false) => ReversalCardMode::SuperSingle,
        (false, true) => ReversalCardMode::ReversalRepeated,
        (false, false) => ReversalCardMode::ReversalSingle,
    };
    let kind = match mode {
        ReversalCardMode::SuperRepeated | ReversalCardMode::ReversalRepeated => {
            AdviceKind::Diagnosis
        }
        ReversalCardMode::SuperSingle | ReversalCardMode::ReversalSingle => AdviceKind::Observation,
    };
    ...
    Some(AdviceCard {
        ...
        title: match mode {
            ReversalCardMode::SuperRepeated => "SA/CAの後隙を繰り返し反撃されている",
            ReversalCardMode::SuperSingle => "SA/CAの後隙に反撃を受けた場面",
            ReversalCardMode::ReversalRepeated => "無敵技という防御回答を繰り返し狩られている",
            ReversalCardMode::ReversalSingle => "無敵技を狩られた場面",
        }.to_string(),
        severity: hp_lost + 0.02 * occurrence_count as f32,
        hp_lost: Some(hp_lost),
        description: match mode {
            ReversalCardMode::SuperRepeated => format!(
                "ゲージ消費で確認したSA/CAが通らず、その後に反撃を受けた場面が {occurrence_count} 回、合計 {:.0}% あります。SA/CAという高コストの回答で複数回損失が出ているため、使用場面を見直す候補です。",
                hp_lost * 100.0
            ),
            ReversalCardMode::SuperSingle => format!(
            ...
            ReversalCardMode::ReversalRepeated => format!(
            ...
            ReversalCardMode::ReversalSingle => format!(
        ...
        practice: match mode {
            ReversalCardMode::SuperRepeated => "各クリップで、コンボ確認・確定反撃・切り返し・単発のどの文脈だったかと、相手にガードされたか即時接触が無かったかを確認します。同じ文脈で反撃されている場合だけ、確認してからSAへつなぐ練習や切り返しの選択比率を調整しましょう。",
            ReversalCardMode::SuperSingle => "クリップで、ヒット確認や確定反撃として撃ったのか、切り返し・単発だったのかを確認します。意図した読みなら単発の失敗として扱い、確認なしで撃っていた場合だけ同じ状況をトレーニングモードで再現しましょう。",
            ReversalCardMode::ReversalRepeated => "起き攻めや固めを記録し、ガード・遅らせ投げ抜け・バックステップ・無敵技を混ぜます。無敵技を同じ局面で連続して選ばず、相手が打撃重ねに偏ったと確認したときだけ比率を上げましょう。",
            ReversalCardMode::ReversalSingle => "クリップで、相手の打撃重ねを読んで撃ったのか、苦しくなって無意識に撃ったのかを確認します。意図した読みなら単発の失敗として扱い、普段も同じ局面で撃っている場合だけ頻度を下げましょう。",
        }.to_string(),
        evidence,
    })
}
