...
pub fn detect_whiff_punished(events: &MatchEvents, own: u8) -> Option<AdviceCard> {
    ...
    Some(AdviceCard {
        ...
        title: match kind {
            AdviceKind::Diagnosis => "届かない技の硬直を繰り返し狩られている",
            _ => "空振りした技の硬直を狩られた場面",
        }
        .to_string(),
        ...
        practice: match kind {
            AdviceKind::Diagnosis => "クリップで、相手のどの位置に対して技を出していたかを確認します。トレーニングで相手を歩かせ、自分の主力技が届く距離と届かない距離の境目を体で覚えてから、その手前で振る練習をしましょう。",
            _ => "クリップで、間合いを測る意図の空振りだったか、届くつもりで外したかを確認します。届くつもりだった場合だけ、その技の実際のリーチを確認しましょう。",
        }
        .to_string(),
        evidence: punished
            .iter()
