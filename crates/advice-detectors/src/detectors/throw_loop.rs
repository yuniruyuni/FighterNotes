...
pub fn detect_throw_loop(events: &MatchEvents, opponent: u8) -> Option<AdviceCard> {
    ...
    for event in &attempts {
        if event.connected {
            ...
            current.push(event);
            best_streak_len = best_streak_len.max(current.len());
        } else {
            current.clear();
        }
    }
    let repeated = best_streak_len as u32 >= THROW_STREAK_MIN;
    ...
    Some(AdviceCard {
        ...
        title: match kind {
            AdviceKind::Diagnosis => "投げを連続して受けている",
            _ => "投げを受けた場面",
        }.to_string(),
        ...
        description: if repeated {
            format!(
                ...
                attempts.len(), connected.len(), best_streak_len
            )
        } else {
            format!(
                "相手の投げ {} 回中 {} 回が通り、この試合の最大連続被投げは {} 回です。投げは打撃との読み合いなので、この結果だけで守り方が悪いとは{OBSERVATION_REVIEW_CAVEAT}。同様に投げられた全 {} 場面を確認できます。",
                attempts.len(), connected.len(), best_streak_len, connected.len()
            )
        },
        practice: match kind {
            AdviceKind::Diagnosis => "打撃重ねと投げをランダム再生し、遅らせ投げ抜け・バックステップ・垂直ジャンプを混ぜます。同じ防御回答を連続して選ばず、まず3連続で投げられないことを目標にしましょう。",
            _ => "クリップで、打撃を警戒してガードを選んだ結果か、毎回同じ守り方をしていたかを確認します。意図した読み負けなら問題ありません。",
        }.to_string(),
