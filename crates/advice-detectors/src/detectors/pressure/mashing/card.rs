...
pub fn build(hits: Vec<MashHit>) -> Option<AdviceCard> {
    if hits.is_empty() {
        return None;
    }
    ...
    Some(AdviceCard {
        ...
        title: match kind {
            AdviceKind::Diagnosis => "守勢でボタンを押して繰り返し被弾している",
            _ => "守勢でボタンを押して被弾した場面",
        }.to_string(),
        severity: hp_lost,
        ...
        practice: match kind {
            AdviceKind::Diagnosis => "相手の固めを記録し、まずボタンを押さずガードだけで受け切ります。慣れたら連係の切れ目・投げ・様子見をランダム再生し、最速打撃を同じタイミングで連続して選ばない練習へ進みます。",
            _ => "クリップで、投げを読んで押したのか、連係の切れ目だと思って押したのかを確認します。判断に理由があれば単発の失敗として扱い、普段も同じタイミングで押している場合だけ回答を散らしましょう。",
        }.to_string(),
