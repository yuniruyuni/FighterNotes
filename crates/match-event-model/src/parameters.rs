...
pub use crate::temporal::{FULL_HP, FULL_MIN_RUN};
/// 全快区間同士をマージする条件: 間の最小 HP がこれ以上なら同一ラウンド前区間
/// （遮蔽による全快検出の途切れであって、実ダメージではない）
pub const MERGE_MIN_HP: f32 = 0.85;
/// KO とみなす HP と持続フレーム
pub const KO_HP: f32 = 0.02;
pub const KO_MIN_RUN: usize = 15;
/// ダメージシーケンスのグループ化ギャップ（コンボのヒット間隔上限）
pub const DMG_GAP: usize = 45;
/// SA 暗転をまたぐ場合だけ許す前後の実ゲーム時間。HP 表示の減少完了から
/// キャンセル演出開始までの遅れを含む（検証済み SA 例では実測 48F）。
pub const DMG_GAP_ACROSS_FREEZE: u32 = 49;
/// 1 減少ステップの最小量（読み取りノイズ除去）
pub const DMG_EPS: f32 = 0.0015;
/// シーケンスとして採用する最小合計ドロップ
pub const DMG_MIN_DROP: f32 = 0.015;
...
pub const JUMP_CONFIRM_BACK: u32 = 15;
pub const JUMP_CONFIRM_FWD: u32 = 8;
...
pub const JUMP_CONFIRM_BACK_GF: i64 = 2;
