...
pub struct JumpEvent {
    ...
    #[serde(default)]
    pub air_end: u32,
    /// 接触スパークの高さ。地上にいる防御側(ジャンプしていない側)の
    /// 足元を 0、頭を 1 とした身長単位で表す。画面座標そのままではなく
    /// 防御側の体で正規化するため、カメラのズームに左右されない。
    /// 空間観測でスパークと防御側の接地追跡が揃った接触だけ値を持つ。
    #[serde(default)]
    pub contact_height: Option<f32>,
    pub round_no: u32,
}
