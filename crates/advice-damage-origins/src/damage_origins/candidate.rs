...
pub fn threat_confidence(confidence: f32) -> Option<EventConfidence> {
    if confidence >= 0.85 {
...
pub fn contact_matches(damage: &DamageEvent, frame: u32) -> bool {
    damage.start_frame.saturating_add(CONTACT_BACK) >= frame
        && damage.start_frame <= frame.saturating_add(CONTACT_DAMAGE_WINDOW)
}

pub fn starts_in(damage: &DamageEvent, start: u32, end: u32) -> bool {
    damage.start_frame >= start && damage.start_frame <= end
}

pub fn approximately_same_drop(actual: f32, attributed: f32) -> bool {
