...
pub const MIN_DETECTOR_COVERAGE_PERCENT: u64 = 60;
/// SAの単フレーム値は演出やラベル遮蔽でuncertainになりやすい一方、
/// 時系列確定層は長い区間に散在する確実値から消費を検証できる。
pub const MIN_SUPER_COVERAGE_PERCENT: u64 = 20;
/// 空間カードは全候補フレームの連続認識ではなく、各イベント近傍の
/// 安定した距離サンプルで確定する。実動画で追跡可能な割合を踏まえた下限。
pub const MIN_SPATIAL_COVERAGE_PERCENT: u64 = 20;

pub fn detector_coverage_is_sufficient(observed: u32, total: u32) -> bool {
    total > 0 && u64::from(observed) * 100 >= u64::from(total) * MIN_DETECTOR_COVERAGE_PERCENT
}

pub fn super_coverage_is_sufficient(observed: u32, total: u32) -> bool {
    total > 0 && u64::from(observed) * 100 >= u64::from(total) * MIN_SUPER_COVERAGE_PERCENT
}

pub fn spatial_coverage_is_sufficient(observed: u32, total: u32) -> bool {
