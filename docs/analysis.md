...
判定は決定的なルールベース処理で、現在の `RULESET_VERSION` は 22。
...
追跡はフレーム間差分だけに依存しない。床帯の局所コントラストから接地影を検出して
足元 anchor を補正し、モーションを作らない静止中の本体は前回観測時のセル外観との
一致で「静止の確認」として維持する。両者が 1 つのモーション領域へ合体した場合は、
領域の中心へ anchor を動かさず各トラックの x を保つ。空中判定の基準は
フレームメーター除外帯の直上に置き、除外帯が足を切った胴体観測を空中と誤読しない。

第一段の contact イベント(hitstop)がかかるフレームには window へ contact ヒントを
付与する。hitstop 中は両者とカメラが静止して差分にはエフェクトだけが残るため、
このヒントのある間に限り、スパーク色の領域の重心を打撃の衝突位置として観測する。
スパーク色はヒット時の「明るく彩度の高い暖色」と、ガード時の「非常に明るく低彩度で
青寄りの寒色」の 2 系統を持ち、後者は実録画のスパークと壁・衣装のセル統計から
較正した(warm-gray の明部は b < r で分離できる)。ヒントの無いフレームでは同種の
発光をノイズとして扱い、彩度の高い衣装が凍結中のトラックへ吸着することも防ぐ。
立ち姿勢の頭より明確に上に出たスパークは空中での接触の傍証とし、体の追跡が演出で
切れた jump 判定の確認にだけ使う(降格には使わない)。

スパークの座標は確認の真偽だけでなく、位置の実数としても消費する。jump の確定接触では
スパーク重心の高さを「地上にいる防御側の身長単位(足元 0、頭 1)」へ正規化して記録し、
対空の引きつけ/飛び込みの当て高さの統計(高め境界 = 身長の 5/4)に使う。画面座標の
ままではカメラのズームに揺れるため、同じ画に写る体を物差しにする。ガードされた攻撃では
スパークと攻撃側 anchor の水平距離を同じ身長単位で測り、1/2 身長以内なら技が根元で
当たった「めり込み」として、距離帯では断定できなかった確反見逃しの reachability を
確認する(先端ガードは何も断定しない。確認専用で、届かない側へ倒すことはない)。

Round 開始直後は側の入れ替わりが起きないため、その確定区間で各プレイヤーの
モーション領域の平均色を学習し、window を跨いで保持する。めくり後など側が
入れ替わった状態から始まる window の初期割り当てでは、学習した色の対応が明確に
逆を示すときだけ「左=P1」の仮定を覆す。

背景と床の水平ストリップを前フレームと相関し、カメラのパン量とズーム比を
毎フレーム推定して観測に含める。SF6 のカメラは距離に応じてズームするため、
screen 座標の距離はゲーム内距離と比例しない。Drive Rush と前ステップ投げの
「距離を詰めた」判定は、このズーム比で端点を同じ縮尺へ戻してから比較する。

クランプの無いカメラは常に両者の中点を画面中央へ寄せるため、中点が中央から
大きく外れた画は壁でパンが止まっていることを意味する。この幾何から、片側が
画面端を背負っている区間を corner span として抽出する(最大ズームアウトと
混同しないよう Far バンドでは判定しない)。span は候補 window 内でしか
観測できないため、端での被弾統計は下限値として扱う。

SA 暗転やカットなどの全画面演出が contact ヒントを潰した場合は、演出明けの
数フレームまでスパークの探索を延長する。演出そのものはスパーク色の割合と
凝集のゲートが弾くため、検出自体は止めない(大技のヒット VFX は画面の
大半を覆うことがあり、それは正しい衝突の画である)。
...
- 画面上の相対距離は camera movement、effect、遮蔽の影響を受ける。カメラのパン・ズームは
  背景相関で毎フレーム推定するが、ゲーム内距離への較正は未実施で、距離バンドは screen 座標の
  閾値のままである。
