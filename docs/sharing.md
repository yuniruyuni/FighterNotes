...
| `superArts` | ruleset v9 の両者のSA1/2/3/CA使用数と結果、自分側だけの利用文脈。各側にavailabilityを持つ |
...
最大欠測区間、ラウンド開始・終了フレームの観測条件をすべて満たす側だけ`available`にする。
欠落フレーム、非試合画面、不確実な読みはすべて欠測として扱う。単発の信頼フレームや検出済みSAイベントだけでは、
ほかの使用を見逃していないと保証できないため`available`にしない。
条件を満たさない側は`unavailable`だけを公開し、件数0として扱わない。
`available`の場合だけ、level、結果、自分側の文脈の全countを必須にする。
