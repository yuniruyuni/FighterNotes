...
import type { RateLimitDecision } from "../usecases/services";
...
export function registerPublishedAnalysisRoutes(app: Hono, ctx: Context) {
  ...
  app.get("/s/:id", async (c) => {
    ...
    try {
      rateLimit = await ctx.services.sharingRateLimit.consume(
        "public_read",
        requestClientKey(
          c.req.raw.headers,
          config.sharing.trustCloudflareConnectingIp,
        ),
        config.sharing.getRateLimit,
      );
    } catch {
